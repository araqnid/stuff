...
public class ResteasyJackson2Provider extends JacksonJaxbJsonProvider {
	...
	public Object readFrom(Class<Object> type,
		...
		if (jp == null || jp.nextToken() == null) { return null; }
		// [Issue#1]: allow 'binding' to JsonParser
		if (((Class<?>) type) == JsonParser.class) { return jp; }
	...
	public void writeTo(Object value,
		...
		/*
		 * 27-Feb-2009, tatu: Where can we find desired encoding? Within HTTP headers?
		 */
		...
		try {
			...
			if (genericType != null && value != null) {
				/*
				 * 10-Jan-2011, tatu: as per [JACKSON-456], it's not safe to just force root type since it prevents polymorphic type
				 * serialization. Since we really just need this for generics, let's only use generic type if it's truly generic.
				 */
				if (genericType.getClass() != Class.class) { // generic types are other impls of 'java.lang.reflect.Type'
					/*
					 * This is still not exactly right; should root type be further specialized with 'value.getClass()'? Let's see how well
					 * this works before trying to come up with more complete solution.
					 */
					rootType = writer.getTypeFactory().constructType(genericType);
					/*
					 * 26-Feb-2011, tatu: To help with [JACKSON-518], we better recognize cases where type degenerates back into
					 * "Object.class" (as is the case with plain TypeVariable, for example), and not use that.
					 */
