...
public class InfoResourcesIntegrationTest {
	...
	public void version_resource() throws Exception {
		CloseableHttpResponse response = doGet("/_api/info/version");
		assertThat(response.getStatusLine(), is(ok()));
		assertThat(
				response,
				is(responseWithJsonContent(jsonObject().withProperty("title", Matchers.notNullValue(TreeNode.class))
	...
	public void health_resource() throws Exception {
		CloseableHttpResponse response = doGet("/_api/info/health");
		assertThat(response.getStatusLine(), is(ok()));
		assertThat(response, is(responseWithJsonContent(any(TextNode.class))));
		response.close();
	}

	@Test
	public void state_resource() throws Exception {
		CloseableHttpResponse response = doGet("/_api/info/state");
		assertThat(response.getStatusLine(), is(ok()));
		assertThat(response, is(responseWithJsonContent(any(TextNode.class))));
		response.close();
	}
	...
	public void routing_resource() throws Exception {
		CloseableHttpResponse response = doGet("/_api/info/routing");
		assertThat(response.getStatusLine(), is(ok()));
		assertThat(response, is(responseWithJsonContent(any(ObjectNode.class))));
		response.close();
	}
